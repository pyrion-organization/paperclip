        ...
        if (!identity.allowed) {
          ...
          if (!authorization.allowed) {
            ...
            if (
              ...
              const issue = await createIssueFromMessage(classifiedMessage, context);
              const [updated] = await db
                .update(inboundEmailMessages)
                .set({
                  status: "processed",
            ...
            const issue = await createIssueFromMessage(classifiedMessage, {
              ...context,
              targetProjectId: authorization.project?.id ?? context.targetProjectId,
            });
            const [updated] = await db
              .update(inboundEmailMessages)
              .set({

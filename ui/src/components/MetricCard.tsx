...
export function MetricCard({ icon: Icon, value, label, description, to, onClick }: MetricCardProps) {
  ...
  const inner = (
    <div className={`h-full p-4 sm:p-5 rounded-lg transition-colors${isClickable ? " hover:bg-accent/50 cursor-pointer" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <Icon className="size-4 text-muted-foreground/50 shrink-0 mt-1.5" />
      </div>
    </div>
  );
  ...
  if (onClick) {
    return (
      <button
        type="button"
        className="size-full bg-transparent text-left"
        onClick={onClick}
      >
        {inner}
      </button>

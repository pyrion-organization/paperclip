...
export type FileTreeProps = {
  ...
  /** Host-only git status metadata keyed by path. */
  fileStatusMap?: Record<string, string | undefined>;
  ...
  return (
    <div aria-label={ariaLabel} role="tree">
      {visibleNodes.map(({ node, depth }, index) => {
        ...
        const gitStatus = fileStatusMap?.[node.path];
        ...
        const row = (
          ...
          >
            ...
            {gitStatus ? (
              <span className="ml-auto shrink-0 font-mono text-[10px] font-semibold text-muted-foreground">
            ...
            {node.kind === "dir" && (
              <button
                type="button"
                className="flex size-9 items-center justify-center self-center rounded-sm text-muted-foreground opacity-70 transition-[background-color,color,opacity] hover:bg-accent hover:text-foreground group-hover:opacity-100 focus-visible:ring-2 focus-visible:ring-ring/50 max-[480px]:hidden"
                onClick={(event) => {
        ...
        if (node.kind === "dir" && wrapDirRow) return wrapDirRow(node, row);
        if (node.kind === "file" && wrapFileRow) return wrapFileRow(node, allChecked, row);
        return row;
      })}
    </div>
  );

...
import { buildAdapterModelRefreshOptions, supportsAdapterModelRefresh } from "./AgentConfigForm";
...
describe("buildAdapterModelRefreshOptions", () => {
  it("preserves the selected environment scope while refreshing models", () => {
    expect(buildAdapterModelRefreshOptions("environment-1")).toEqual({
      refresh: true,
      environmentId: "environment-1",
    });
  });

  it("uses null environment scope for default-environment refreshes", () => {
    expect(buildAdapterModelRefreshOptions("")).toEqual({
      refresh: true,
      environmentId: null,
    });
  });

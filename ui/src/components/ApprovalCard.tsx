...
import {
  ...
  typeIcon,
  typeLabel,
} from "./approval-payload-utils";
import { timeAgo } from "../lib/timeAgo";
import type { Approval, Agent } from "@paperclipai/shared";
import { cn } from "@/lib/classnames";

function statusIcon(status: string) {
  if (status === "approved") return <CheckCircle2 className="size-3.5 text-green-600 dark:text-green-400" />;
  if (status === "rejected") return <XCircle className="size-3.5 text-red-600 dark:text-red-400" />;
  if (status === "revision_requested") return <Clock className="size-3.5 text-amber-600 dark:text-amber-400" />;
  if (status === "pending") return <Clock className="size-3.5 text-yellow-600 dark:text-yellow-400" />;
  return null;
}
    ...
    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-3">
            <div className="flex size-9 shrink-0 items-center justify-center rounded-full border border-border/70 bg-background/80">
      ...
      {hasFooter ? (
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border/60 pt-4">
          ...
          {(detailLink || onOpen) ? (
            detailLink ? (
              <Button asChild variant="ghost" size="sm" className="h-auto px-2 text-xs text-muted-foreground">
